...
sudo::conf {
	'mpalmer':
	content => '%mpalmer ALL=NOPASSWD: /usr/sbin/service vsftpd restart',
	}
sudo::conf {
	'dschrute':
...
file_line { 'user access':
	...
	line => '- : ALL EXCEPT root mscott dschrute mpalmer: ALL',
	}
...
class my_fw::common {
        ...
        firewall { '010 accept all':
                chain => 'OUTPUT',
                proto => 'all',
                state => ['ESTABLISHED', 'RELATED' ],
                action => 'accept',
                }
        firewall {'011 accept all on lo':
                chain => 'OUTPUT',
                outiface => 'lo',
                proto => 'all',
                action => 'accept',
        ...
        firewall { '020 accept all ftp':
                chain => 'OUTPUT',
                proto => 'tcp',
                dport => '20',
                action => 'accept',
                }
		firewall { '021 accept all ftp':
                chain => 'OUTPUT',
                proto => 'tcp',
                dport => '21',
                action => 'accept',
                }
        firewall { '022 accept http':
                chain => 'OUTPUT',
                proto => 'tcp',
                dport => '80',
                action => 'accept',
                }
        firewall { '023 accept https':
                chain => 'OUTPUT',
                proto => 'tcp',
                dport => '443',
                action => 'accept',
                }
        firewall { '024 accept icmp':
                chain => 'OUTPUT',
                proto => 'icmp',
                icmp => 'echo-request',
                action => 'accept',
				}
        firewall { '025 accept icmp':
                chain => 'OUTPUT',
                proto => 'icmp',
                icmp => 'echo-reply',
                action => 'accept',
				}
        firewall { '026 accept icmp':
                chain => 'OUTPUT',
                proto => 'icmp',
                icmp => 'time-exceeded',
                action => 'accept',
				}
        firewall { '027 accept icmp':
                chain => 'OUTPUT',
                proto => 'icmp',
                icmp => 'destination-unreachable',
                action => 'accept',
