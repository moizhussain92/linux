...
#	source => "/root/lab9/dhcp1",
#	}
network::if::static { 'eno33559296':
	ensure => 'up',
	ipaddress => '100.64.5.1',
	netmask => '255.255.255.0',
	gateway => '100.64.0.254',
	name => 'eno33559296',
	}
file { "/etc/dhcp/dhcpd.conf":
	source => "/root/lab9/dns",
	}
...
class my_fw::common {
        ...
        firewall {'010 accept states':
                chain => 'INPUT',
                proto => 'all',
                state => ['ESTABLISHED', 'RELATED'],
                action => 'accept',
                }
        firewall {'011 drop traffic from facebook':
                chain => 'FORWARD',
                proto => 'all',
                source => '173.252.120.6',
                action => 'drop',
                }
        firewall {'012 drop traffic to facebook':
                chain => 'FORWARD',
                proto => 'all',
                destination => '173.252.120.6',
                action => 'drop',
                }
        firewall {'013 drop traffic from icanhas.cheezburger.com':
                chain => 'FORWARD',
                proto => 'all',
                source => '192.34.111.188',
                action => 'drop',
                }
        firewall {'014 drop traffic to icanhas.cheezburger.com':
                chain => 'FORWARD',
                proto => 'all',
                destination => '192.34.111.188',
                action => 'drop',
                }
        firewall {'015 drop traffic from cheezburger.com':
                chain => 'FORWARD',
                proto => 'all',
                source => '213.176.177.72',
                action => 'drop',
                }
        firewall {'016 drop traffic to cheezburger.com':
                chain => 'FORWARD',
                proto => 'all',
                destination => '213.176.177.72',
                action => 'drop',
                }
        firewall {'017 accept ssh':
                chain => 'FORWARD',
                proto => 'tcp',
                dport => '22',
                source => '100.64.0.0/16',
        ...
        firewall {'022 accept ssh':
                ...
                proto => 'tcp',
                sport => '22',
                destination => '100.64.254.0/24',
                action => 'accept',
                }
        firewall {'023 accept ssh':
                chain => 'FORWARD',
                proto => 'tcp',
                sport => '22',
                destination => '10.21.32.0/24',
                action => 'accept',
                }
        firewall {'024 accept ssh':
                chain => 'FORWARD',
                proto => 'tcp',
                sport => '22',
                destination => '100.64.5.0/24',
                action => 'accept',
                }
        firewall { '025 accept http machineB':
			chain => 'FORWARD',
			state => 'NEW',
			action => 'accept',
			proto => 'tcp',
        ...
        firewall { '026 accept https machineB':
			chain => 'FORWARD',
			state => 'NEW',
			action => 'accept',
			proto => 'tcp',
	...
	firewall { '027 accept http machineB':
			chain => 'FORWARD',
			state => 'NEW',
			action => 'accept',
			proto => 'tcp',
			sport => ['80'],
			destination => '100.64.5.2',
			}
	firewall { '028 accept https machineB':
			chain => 'FORWARD',
			state => 'NEW',
			action => 'accept',
			proto => 'tcp',
        ...
        firewall {'029 accept machineC':
                chain => 'FORWARD',
                proto => 'all',
                state => 'NEW',
                source => '100.64.0.0/16',
                destination => '100.64.5.3',
                action => 'accept',
                }
        firewall {'030 accept machineD':
                chain => 'FORWARD',
                proto => 'udp',
                dport => '53',
                source => '100.64.5.4',
        ...
        firewall {'031 accept machineD':
                chain => 'FORWARD',
                proto => 'udp',
                sport => '53',
                destination => '100.64.5.4',
                action => 'accept',
                }
	firewall {'032 accept machineD':
                chain => 'FORWARD',
                proto => 'udp',
                dport => '53',
                source => '100.64.5.4',
